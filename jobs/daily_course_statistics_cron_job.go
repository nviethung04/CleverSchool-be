...
func NewDailyCourseStatisticsCronJob() *DailyCourseStatisticsCronJob {
	return &DailyCourseStatisticsCronJob{}
}

func (j *DailyCourseStatisticsCronJob) initService() error {
	if j.service != nil {
		return nil
	}
	...
	repository := repositories.NewDashboardReportCoursesRepository()
	j.service = services.NewDashboardReportCoursesService(repository)

	return nil
}

// Run chạy job hàng ngày với 4 khoảng thời gian khác nhau
func (j *DailyCourseStatisticsCronJob) Run() error {
	...
	fmt.Printf("\n🎉 Hoàn thành Daily Course Statistics Job\n")
	return nil
}

// getStartOfWeek lấy ngày đầu tuần (Thứ 2)
func (j *DailyCourseStatisticsCronJob) getStartOfWeek(t time.Time) time.Time {
...
func (j *DailyCourseStatisticsCronJob) getEndOfWeek(t time.Time) time.Time {
	weekday := int(t.Weekday())
	if weekday == 0 { // Chủ nhật
		return t
	}
