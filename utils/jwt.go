...
func GenerateToken(userID, roleID, schoolID int, roleName string, roleIDs []int, roles []string, roleType string, memberType string) (string, error) {
	claims := CustomClaims{
		UserID:     userID,
		RoleID:     roleID,
		SchoolID:   schoolID,
...
// CheckTokenForWebSocket validates JWT for WebSocket connections
// Only checks signature and expiry, no session store validation
// This prevents WS auth failures when session cleanup happens
func CheckTokenForWebSocket(tokenStr string) (*CustomClaims, error) {
	token, err := jwt.ParseWithClaims(tokenStr, &CustomClaims{}, func(token *jwt.Token) (interface{}, error) {
		return jwtSecret, nil
	})
	if err != nil {
