...
type GetExerciseAnswersService interface {
	GetExerciseAnswers(ctx context.Context, req *prot.GetExerciseAnswersRequest) (*prot.GetExerciseAnswersResponse, error)
}

type getExerciseAnswersService struct {
	repo repositories.GetExerciseAnswersRepository
}

func NewGetExerciseAnswersService(repo repositories.GetExerciseAnswersRepository) GetExerciseAnswersService {
	return &getExerciseAnswersService{
		repo: repo,
	}
}

func (s *getExerciseAnswersService) GetExerciseAnswers(ctx context.Context, req *prot.GetExerciseAnswersRequest) (*prot.GetExerciseAnswersResponse, error) {
	clonedRepo := repositories.NewClonedQuestionRepository()
	clonedQuestion, err := clonedRepo.FindByAssignment(req.ExerciseId, models.ClonedQuestionTypeExercise)
	...
	duration, _, submittedAt, hasManualScoring, ratio, _ := s.repo.GetExerciseUserInfo(req.ExerciseId, req.UserId)
	// totalQuestions lấy từ cloned_question
	totalQuestions := len(rawQuestions)
	unscoredQuestions, _ := s.repo.GetUnscoredManualQuestions(req.ExerciseId, req.UserId)
	comment, _ := s.repo.GetExerciseComment(req.ExerciseId, req.UserId)

	overview := &prot.ExerciseAnswerOverview{
	...
	var questionAnswers []*prot.QuestionAnswerExercise
	for _, question := range questions {
		...
		questionAnswer := &prot.QuestionAnswerExercise{
			Id:           question.Id,
			Content:      contentText,
			Type:         question.Type,
			FileUrl:      fileURL,
		...
		case "multiple_choice":
			answers, err := s.repo.GetMultipleChoiceAnswerExercise(req.ExerciseId, question.Id, req.UserId)
			...
			if err != nil || len(answers) == 0 {
				...
				answerMultipleChoice := &prot.AnswerMultipleChoiceExercise{
					AnswerIds:          []int64{},
				...
				questionAnswer.Answer = &prot.QuestionAnswerExercise_AnswerMultipleChoiceExercise{
					AnswerMultipleChoiceExercise: answerMultipleChoice,
				}
			} else {
				allAnswerIds := []int64{}
				allAnswerContents := []string{}
		...
		case "fill_in_blanks":
			...
			answers, err := s.repo.GetFillInBlankAnswerExercise(req.ExerciseId, question.Id, req.UserId)
			if err != nil {
				questionAnswer.Answer = &prot.QuestionAnswerExercise_AnswerFillInBlankExercise{
					AnswerFillInBlankExercise: &prot.AnswerFillInBlankExercise{
						Answers:      []*prot.FillInBlankAnswerExercise{},
			...
			var fillInBlankAnswers []*prot.FillInBlankAnswerExercise
			if err != nil || len(answers) == 0 {
				for _, answer := range question.Options.Answers {
					fillInBlankAnswers = append(fillInBlankAnswers, &prot.FillInBlankAnswerExercise{
		...
		case "ordering", "drag_drop":
			var isAllCorrect bool
			answers, err := s.repo.GetPositionAnswerExercise(req.ExerciseId, req.UserId, question.Id)
			if err != nil {
				questionAnswer.Answer = &prot.QuestionAnswerExercise_AnswerPositionExercise{
					AnswerPositionExercise: &prot.AnswerPositionExercise{
						Answers:      []*prot.PositionAnswerExercise{},
						IsAllCorrect: isAllCorrect,
					},
				}
				config.Log.Error("GetPositionAnswerExercise: ", err)
				break
			}

			var positionAnswers []*prot.PositionAnswerExercise
			if err != nil || len(answers) == 0 {
				for _, answer := range question.Options.Answers {
					if answer.GroupPosition > 0 {
						positionAnswers = append(positionAnswers, &prot.PositionAnswerExercise{
							SortPosition:        answer.CorrectPosition,
		...
		case "matching":
			var isAllCorrect bool
			answers, err := s.repo.GetMatchingAnswerExercise(req.ExerciseId, question.Id, req.UserId)

			if err != nil {
				questionAnswer.Answer = &prot.QuestionAnswerExercise_AnswerMatchingExercise{
					AnswerMatchingExercise: &prot.AnswerMatchingExercise{
						Matches:      []*prot.MatchingPairAnswerExercise{},
						IsAllCorrect: isAllCorrect,
					},
				}
			...
			var matches []*prot.MatchingPairAnswerExercise
			if err != nil || len(answers) == 0 {
				for _, source := range question.Options.Sources {
					...
					matches = append(matches, &prot.MatchingPairAnswerExercise{
		...
		case "labeling":
			var isAllCorrect bool
			answers, err := s.repo.GetLabelingAnswerExercise(req.ExerciseId, question.Id, req.UserId)
			if err != nil {
				questionAnswer.Answer = &prot.QuestionAnswerExercise_AnswerLabelingExercise{
					AnswerLabelingExercise: &prot.AnswerLabelingExercise{
						Answers:      []*prot.LabelingAnswerExercise{},
						IsAllCorrect: isAllCorrect,
					},
				}
				config.Log.Error("GetLabelingAnswerExercise: ", err)
				continue
			}

			var labelingAnswers []*prot.LabelingAnswerExercise
			if err != nil || len(answers) == 0 {
				for _, label := range question.Options.Labels {
					...
					labelingAnswers = append(labelingAnswers, &prot.LabelingAnswerExercise{
						BlankId:             blankId,
		...
		case "category":
			answers, err := s.repo.GetGroupAnswerExercise(req.ExerciseId, question.Id, req.UserId)
			if err != nil {
				questionAnswer.Answer = &prot.QuestionAnswerExercise_AnswerGroupExercise{
					AnswerGroupExercise: &prot.AnswerGroupExercise{
						Groups:       []*prot.CategoryAnswerExercise{},
						IsAllCorrect: false,
					},
				}
			...
			var categoryAnswers []*prot.CategoryAnswerExercise
			isAllCorrect := true
			...
			if err != nil || len(answers) == 0 {
				for _, item := range question.Options.Items {
					...
					categoryAnswers = append(categoryAnswers, &prot.CategoryAnswerExercise{
		...
		case "writing", "speaking":
			answer, err := s.repo.GetManualAnswerExercise(req.ExerciseId, question.Id, req.UserId)
			if err != nil {
				questionAnswer.Answer = &prot.QuestionAnswerExercise_AnswerManualExercise{
					AnswerManualExercise: &prot.AnswerManualExercise{
						Answer:        "",
						AnswerFileUrl: "",
						Score:         0,
						IsScored:      false,
	...
	return &prot.GetExerciseAnswersResponse{
