...
type DashboardStudentExerciseService interface {
	GetStudentExerciseStats(userID int64, courseID *int64, startDate, endDate *int64) (*prot.DashboardStudentHomeworkResponse, error)
}

type dashboardStudentExerciseService struct {
	repo repositories.DashboardStudentExerciseRepository
}

func NewDashboardStudentExerciseService(repo repositories.DashboardStudentExerciseRepository) DashboardStudentExerciseService {
	return &dashboardStudentExerciseService{repo: repo}
}

func (s *dashboardStudentExerciseService) GetStudentExerciseStats(userID int64, courseID *int64, startDate, endDate *int64) (*prot.DashboardStudentHomeworkResponse, error) {
	repoResult, err := s.repo.GetStudentExerciseStats(userID, courseID, startDate, endDate)
	if err != nil {
		return nil, err
	}
